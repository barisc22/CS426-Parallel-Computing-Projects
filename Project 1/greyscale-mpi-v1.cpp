	...
	if (rank == 0) {
		...
		for (int i = 1; i < sizeS - 1; i++) {
			...
			for (int j = 0; j < arrSize1; j++) {
				arrForGoGeneral[j] = (char*)malloc(13);
				arrForGoGeneral[j] = image[(i * arrSize1) + j];
				MPI_Send(arrForGoGeneral[j], 13, MPI_CHAR, i, 0, MPI_COMM_WORLD);
			}
		}

		MPI_Send(sizeHop1, 1, MPI_INT, sizeS - 1, 0, MPI_COMM_WORLD);
		for (int i = (arrSize1 * (sizeS - 1)); i < sum - 1; i++) {
			arrForGoRemainder[dummyVar] = (char*)malloc(13);
			arrForGoRemainder[dummyVar] = image[i];
			MPI_Send(arrForGoRemainder[dummyVar], 13, MPI_CHAR, sizeS - 1, 0, MPI_COMM_WORLD);
			dummyVar++;
		}
		...
		printf("Parallel Image ConversionV1 took %.5f seconds\n", end - start);
	}

	else if (rank > 0 && rank < sizeS) {
		int arrNo[1];
